...
export function PuntosFidelizacionPanel({ showHeaderActions = true }: { showHeaderActions?: boolean }) {
  ...
  // Vincular con clientes del CRM si existen (mismo nombre exacto)
  const [crmClientes] = useLocalValue<Array<{ id: string; nombre: string }>>("vertice.clientes", []);
  ...
  return (
    <div className="space-y-6">
      ...
      <CompraDialog
        ...
        config={config}
        crmNombres={crmClientes.map(c => c.nombre)}
        onSave={(nombre, monto, servicio, fecha) => {
          // Vincular con CRM si existe el mismo nombre (case-insensitive)
          const match = crmClientes.find(cl => cl.nombre.trim().toLowerCase() === nombre.trim().toLowerCase());
          const nombreFinal = match?.nombre ?? nombre.trim();
          const id = normalize(nombreFinal);
          const puntos = Math.floor(monto / config.montoPorPunto);
          const prev = clientes[id];
          const nuevoCliente: Cliente = {
            nombre: prev?.nombre ?? nombreFinal,
...
function CompraDialog({ open, onClose, config, crmNombres, onSave }: {
  open: boolean; onClose: () => void; config: Config; crmNombres: string[];
  ...
  const sugerencias = nombre.trim().length >= 2
    ? crmNombres.filter(n => n.toLowerCase().includes(nombre.toLowerCase()) && n.toLowerCase() !== nombre.toLowerCase()).slice(0, 5)
    : [];
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        ...
        <div className="space-y-3">
          <div>
            <Label>Cliente</Label>
            <Input value={nombre} onChange={e => setNombre(e.target.value)} placeholder="Nombre del cliente" list="crm-clientes-pc" />
            <datalist id="crm-clientes-pc">
              {crmNombres.map(n => <option key={n} value={n} />)}
            </datalist>
